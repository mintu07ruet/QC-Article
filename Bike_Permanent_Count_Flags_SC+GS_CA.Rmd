...
input_data <- read_csv("C:/UC Berkeley Project Backup/AT Census/Processed Data/Eco_combined_data_rural_vs_urban_before_QA_QC.csv")
...
```
# above weekday graph confirms that 1000 is the valid hourly count for the study area in weekday
...
ggplot(weekend) +
    geom_point(aes(x = ID, y = counts))
```
# We will aslo consider 1000 as the valid hourly count in weekends
# Find the upper threshold for weekdays and weekends

```{r}
weekday1$upper_threshold==1000
weekend1$upper_threshold==1000
weekday2=weekday1%>% mutate(mean_hourly_volume=mean(counts)) %>%
  ...
  mutate(max_threshold=1000*mean_hourly_ratio*2)
```
`
```{r}
...
input_data_flagged <- input_data2 %>%
  # 1=weekend, 0=weekday
  mutate(max_flag = ifelse(weekday_Status==1 & counts>1000 | weekday_Status==0 & counts>1000, T, F)) %>% 
  mutate(night_flag=ifelse(hour<6 & hour>21 & counts>10  & max_flag==F, T, F)) %>%
  ...
  mutate(MAD_flag = counts>MAD_flag_threshold & max_flag==F & hour>=6 & hour<=21 & counts>50) %>%
  dplyr::ungroup() %>%
...
```{r}
write.csv(input_data_flagged1,'C:/UC Berkeley Project Backup/AT Census/Lindsey _QA/bike_flagged_data.csv')
write.csv(input_data_unflagged1,'C:/UC Berkeley Project Backup/AT Census/Lindsey _QA/bike_unflagged_data.csv')
```
